<script lang="ts">
	...
	interface Props {
		/** The uploaded file's URL; empty string when there is no image. */
		url: string;
		/** Which file-router endpoint to upload through. Read once, at init. */
		endpoint: Parameters<typeof createUploadThing>[0];
		/** CSS aspect-ratio for the preview, e.g. "16 / 9" or "4 / 5". */
		aspect?: string;
		title?: string;
		hint?: string;
		previewAlt?: string;
		/** Intrinsic dimensions on the preview, to reserve layout space. */
		width?: number;
		height?: number;
	}
	...
	// The endpoint is fixed for the lifetime of a given upload control, so
	// capturing it once here is deliberate.
...
{#if url}
	<figure class="preview">
		<img src={url} alt={previewAlt} {width} {height} style="aspect-ratio: {aspect}" />
